...
NOTE: This measurement only indicates how many different species were found, but doesn't indicate the abundance of them. Therefore there could be regions that are low in 'richness' yet still have high populations of pollinators.
...
Here we can plot Oregon's ecoregions 
...
This says a lot more than the previous visualization. Richness is highest in the Blue Mountains and Eastern Cascades. The coast range also shows lower abundance. Lets incorporate the precipitation data to better understand this. 
...
Now that the precipitation data is loaded in. Below I've done some data manipulation to generate yearly means by ecoregion. In doing so we can compare precipitaion and species richness by ecoregion. 
...
When looking at overall species counts by ecoregion, we can see that there isn't a large difference. Mountainous and remote areas such as the Blue Moutains and Eastern Cascades have the largest counts (399 and 386 respectively), and dry xeric regions have the lowest (Snake River Plain with 99, and Central Basin and Range with 131) 
...
Both the Willamette Valley and Eastern Cascade show negative correlations.

```{r}
ggplot(eco_rich_yr[eco_rich_yr$LEV3_NAME == "Coast Range", ],
       aes(year, richness)) +
  geom_point(size = 2, color = "firebrick") +
  geom_smooth(method = "lm", se = TRUE, fill = "pink") +
  labs(
    title = "Richness by Year: Coast Range",
    x = "Year",
    y = "Richness"
  ) +
  theme_minimal(base_size = 14)
...
From this exploration, although there are differences in Oregon ecoregions when looking at species richness, trends remain inconclusive. Going into this project/question I was expecting the Willamette Valley to have lower species richness, however with 260 total counted species, this puts it right at the average for Oregon. 
...
summary(mod_precip)
```
Here I fitted a linear model with richness as the response and mean precipitation as the predictor. The estimated slope was 0.514. Because the p-value was 0.6087 (much larger than 0.05), I failed to reject my null hypothesis. This suggests that, within this dataset, precipitation doesn't have much of a large effect on species richness. That is if we are looking at Oregon as a whole. 
...
summary(mod_precip)
```
Here the estimated effect are stronger (β = −4.94, std error = 2.59), suggesting that wetter years were associated with slightly lower species richness. However this effect was not statistically significant (t = −1.91, p = 0.1145), and I therefore fail to reject the null hypothesis.

Despite the non-significant result, the model explained a substantial portion of the variation (R² = 0.42), indicating a potentially meaningful relationship that may be obscured by the limited time period. Overall, the evidence isn't strong enough to conclude that precipitation alone predicts richness in the Willamette Valley.
...
Although I wish the result were more exciting, I hope the data explored provided some insight into bee populations in Oregon. Thank you so much for reading this paper.